...
// Función para guardar investigador
export async function guardarInvestigador(datos: any) {
  try {
    ...
    if (correo && correo !== "") {
      const existenteCorreo = await db.get("SELECT * FROM investigadores WHERE correo = ?", correo)
      if (existenteCorreo) {
    ...
    if (!curp || curp === "") {
      const existenteNombre = await db.get("SELECT * FROM investigadores WHERE nombre_completo = ?", nombre)
      if (existenteNombre) {
